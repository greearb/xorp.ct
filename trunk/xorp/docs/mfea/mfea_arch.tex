%
% $XORP: xorp/docs/mfea/mfea_arch.tex,v 1.11 2003/11/06 18:37:42 pavlin Exp $
%
...
Currently (June 2004), the MFEA supports abstraction for
...
path~\footnote{Currently (June 2004) Click does
...
\begin{itemize}
  ...
  multicast-related modules. Note that as of June 2004 this interface is
  deprecated, and the multicast modules interested in
  the MRIB should obtain this information from the RIB module instead.
...
packets~\footnote{In the future (after June 2004), the PIM
...
2004) there are few occasions when XrlMfeaNode uses direct access to MfeaVif.}.
...
(June 2004), MfeaConfig is not implemented; rather, all state is
...
{\bf Note: As of June 2004 obtaning the MRIB information via the MFEA
is deprecated. In the future the MRIB support within the MFEA will
...
The MRIB information is obtained by periodically reading the underlying
unicast forwarding table by directly calling the appropriate method in the
FEA.
...
\begin{itemize}
  ...
  \item June 22, 2004: Updated to match XORP version 1.0 release code.
