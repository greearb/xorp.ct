%
% $XORP: xorp/docs/mfea/mfea_arch.tex,v 1.1.1.1 2002/12/11 23:55:56 hodson Exp $
%
...
issues when implementing a new protocol. In addition, if we want to use
...
Currently (March 2003), the MFEA supports abstraction for
...
path~\footnote{Currently (March 2003) Click does
not support multicast yet.}. Support for other systems will be added
on-demand if there are available resources.

For simplicity, currently (March 2003) the MFEA is a 
separate process from the unicast FEA. One of the reasons for that
...
the PIM-SM daemon for example starts sending a large number of requests
...
XrlMfeaNode implements XRL-based communication mechanisms between MfeaNode
...
packets~\footnote{In the future (after March 2003), the PIM
...
used by the MFEA to keep track of any changes to an interface (\eg an
alias address has been added/deleted, etc). If there is a change to an
interface, the MFEA informs all protocol modules that have enabled the
specific protocol on that interface~\footnote{Currently (March 2003),
the MFEA reads the interface status from the underlying system only once
(on startup); instead, it should keep monitoring the interface status by
periodically reading that status for example, and/or by considering the
information provided by the XORP Router Manager (within the XORP
framework).}.
...
would be through MfeaNode~\footnote{For simplicity, currently (March
...
(March 2003), MfeaConfig is not implemented; rather, all state is
...
the Shortest-Path Tree switc in case of PIM-SM: the SPT switch is
...
Currently (March 2003), the MRIB information is obtained by
periodically reading the underlying unicast forwarding table. In the
...
called UnixComm0 and which is described in
Section~\ref{sec:unixcomm0_description}).

UnixComm implements various UNIX-specific methods that use the appropriate
...
interface), and so on. Typically, there is a single UnixComm entry per
...
MRIB information, etc.~\footnote{Strictly speaking, this information
should not be kept in the system-specific UnixComm entry, but in some
system-independent entry. For simplicity, for now this state is
kept in UnixComm; in the future (after March 2003), UnixComm
...
for the following tasks (this list may not be complete). In other
...
\begin{itemize}
  ...
  system, and report any changes about it (the latter one is not
  implemented yet).

  \item Read the underlying unicast forwarding table, and report
  any changes about it.

  \item Send/receive protocol-specific packets (e.g., PIM or IGMP)
  to/from the network interfaces.
