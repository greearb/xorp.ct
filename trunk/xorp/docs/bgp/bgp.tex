%
% $XORP: xorp/docs/bgp/bgp.tex,v 1.21 2006/08/02 12:39:46 mjh Exp $
%
...
Figure 2 expands on the plumbing to include all the plumbing stages in
the current version of BGP, and broadly groups them according to
whether or not they modify or store routes.
...
framework, which is described in much more detail in the paper
``Decoupling Policy from Protocols'', available from the XORP web
site.  This also describes the distinct roles played by each of the
three PolicyTable stages in the BGP pipeline.
...
\begin{itemize}
  ...
  \item August 2, 2006: Updated the diagrams to include PolicyTable,
  AggregationTable, and DampingTable.  Added more detailed pipeline
  diagram.  Added brief description of PolicyTable.  Added
  ``Modification History'' appendix.  Updated the version to 1.3, and
  the date.
