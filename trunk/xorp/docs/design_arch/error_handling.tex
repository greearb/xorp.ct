%
% $XORP: xorp/docs/design_arch/error_handling.tex,v 1.22 2003/06/03 23:50:23 pavlin Exp $
%
...
\item NO\_FINDER -- This error occurs when a \xorp process cannot
communicate with the \finder. This always indicates a serious problem with
the router, as the \finder should always be present. The application MAY
treat this error as fatal.

\item RESOLVE\_FAILED -- This error occurs when a \xorp process tries to
resolve an XRL relating to a target that does not export any XRLs at the
time of the query. This may occur because the relevant process is not
running, or because it has not finished registering its XRLs with the
\finder.

RESOLVE\_FAILED errors may happen because of a benign cause, namely that
processes started up in a less than perfect order, so a target's user has
initialized before the target itself. Applications SHOULD handle this type
of transient RESOLVE\_FAILED error with a retransmission strategy.
Alternatively, applications may use a \finder interface for detecting when
the relevant targets are ready (yet to be defined), in which case
RESOLVE\_FAILED may be treated as a fatal error.

\item NO\_SUCH\_METHOD -- This error occurs when the named target is
running, but it does not support the method named in the XRL.
...
errors are not transient: If an XRL access gets a NO\_SUCH\_METHOD error,
then that XRL will always result in a NO\_SUCH\_METHOD error, at least
until the target process restarts.
...
SEND\_FAILED errors for class targets (that is, targets that do
...
error -- either REPLY\_TIMED\_OUT, SEND\_FAILED, NO\_FINDER, or
RESOLVE\_FAILED.

\end{itemize}

NO\_FINDER, RESOLVE\_FAILED, and to some extent NO\_SUCH\_METHOD generally
represent serious problems with the router. SEND\_FAILED
represents a serious problem with the target, such as that an instance of
the target has died; this problem may or may not be transient. SEND\_FAILED\_TRANSIENT
and REPLY\_TIMED\_OUT are potentially common errors, and should be handled
by the application. However, the likelihood of SEND\_FAILED\_TRANSIENT can often be
reduced, making it a ``fatal'' error from the application's point of view,
by limiting the rate at which requests are sent.

The first four errors, NO\_FINDER, RESOLVE\_FAILED, NO\_SUCH\_METHOD, and
SEND\_FAILED\_TRANSIENT, occur when there is no chance that the relevant XRL was
communicated to its target. They are therefore called \emph{send failures}.
The other two errors, REPLY\_TIMED\_OUT and SEND\_FAILED, may be
...
If a SEND\_FAILED\_TRANSIENT error occurs, the sending application MAY retransmit the
XRL.
...
whether to retransmit XRLs that return SEND\_FAILED\_TRANSIENT or REPLY\_TIMED\_OUT, but
