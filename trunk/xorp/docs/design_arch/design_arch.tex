%
% $XORP: xorp/docs/design_arch/design_arch.tex,v 1.19 2006/07/05 03:06:28 pavlin Exp $
%
...
\begin{itemize}
  ...
  \item August 28, 2003: Add SNMP to the set of implemented
  items. Update the version to 0.4, and the date.

  \item November 6, 2003: Updated the BGP status (now it supports
  IPv6). Update the version to 0.5, and the date.

  \item July 8, 2004: Updated the status (now there is support for RIP and
  RIPng). Update the version to 1.0, and the date.

  \item April 13, 2005: Updated the status (now there is support for
  Click forwarding path, etc). Update the version to 1.1, and the date.
  ...
  forwarding path). Update the version to 1.2, and the date.

  \item August 2, 2006: Update the status (now XORP implements IGMPv3
  and MLDv2). Update the version to 1.3, and the date.
