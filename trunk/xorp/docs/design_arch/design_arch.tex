%
% $XORP: xorp/docs/design_arch/design_arch.tex,v 1.8 2002/12/11 10:19:32 pavlin Exp $
%
...
wishing to modify the software.  People who are interested in the XORP
...
The primary goal of XORP to fill this gap.  To succeed it must be both
...
most of the code is written to support both, as of Release 0.1
(December 2002) only IPv4 has been tested.
...
\begin{itemize}
  \item {\bf PIM-SM}, PIM-SSM, Bidir-PIM (IPv4 and IPv6)
  \item {\bf IGMPv1, v2}, v3 (IPv4 only)
  \item {\bf MLDv1}, v2 (IPv6 only)
\end{itemize}
...
XORP component in Figure~\ref{fig:process_model} will be run as a
...
modify unicast forwarding entries, etc.
...
modules can communicate directly with an user-space process. However,
...
Note that in release 0.1 (December 2002), the multicast-related
functionalities are separated from the unicast-based functionalities
in the MFEA (Multicast Forwarding Engine Abstraction).
...
appropriate forwarding entries in in the FEs.

The RIB also holds the routing information for multicast-capable routes
(MRIB) to be used for Reverse-Path Forwarding (RPF) information. etc.
...
computed by the particular multicast routing protocol, but this
information is kept by the multicast routing protocol itself, and
...
This is the BGP routing daemon. It implements IPv4 and IPv6
unicast routing, as well as MBGP for both IPv4 and IPv6 multicast RIBs
for multicast routing purpose.
...
Currently (December 2002), we intend to use a port of an existing OSPF
implementation (http://www.ospf.org/) to the XORP architecture.
However this does not support IPv6.
...
Currently (December 2002), we intend to port existing RIP
implementations ({\emph routed} and {\emph route6d} from FreeBSD) to
the XORP architecture.

%%%%%%%%%%%%%%%%%%%%%
\subsubsection{IGMP/MLD}

This is Internet Group Management Protocol/Multicast Listener Discovery
handler. It implements the router-side part of IGMP and MLD. Its main
purpose is to discover local multicast members and propagate this
information to multicast routing daemons such as PIM-SM.

For more information about the XORP IGMP/MLD implementation see
\cite{xorp:mld_igmp}.
...
the IPv4 and IPv6 daemons are separate. The PIM-SM protocol requires
...
The RTRMGR is the process responsible for starting all components of
...
For more information about the RTRMGR see
\cite{xorp:rtrmgr}.

%%%%%%%%%%%%%%%%%%%%%
\subsubsection{CLI: Command Line Interface}

The CLI is intended to be used by an user to access the router, view
its internal state, or to configure it on-the-fly. Its functionality
is closely related to the RTRMGR. However, because the robustness of
the RTRMGR itself is extremely important, all functionality that can
be run as a separate CLI process are separated from the RTRMGR.
...
The IPC finder is needed by the communication method used among all XORP
components, \ie the XRLs. Each of the XORP components registers with the
IPC finder. The finder assists the XRL communications (more specifically,
it knows the location of each XRL target), therefore a XORP process does
not need to know explicitly the location of all other processes, or how
...
requests. Currently (December 2002) it has not been
implemented yet.
