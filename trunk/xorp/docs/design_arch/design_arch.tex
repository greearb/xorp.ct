%
% $XORP: xorp/docs/design_arch/design_arch.tex,v 1.20 2006/08/01 19:49:27 pavlin Exp $
%
...
\begin{itemize}
  ...
  \item August 28, 2003: Added SNMP to the set of implemented
  items. Updated the version to 0.4, and the date.

  \item November 6, 2003: Updated the BGP status (now it supports
  IPv6). Updated the version to 0.5, and the date.

  \item July 8, 2004: Updated the status (now there is support for RIP and
  RIPng). Updated the version to 1.0, and the date.

  \item April 13, 2005: Updated the status (now there is support for
  Click forwarding path, etc). Updated the version to 1.1, and the date.

  \item March 8, 2006: Updated the status (now XORP has its own OSPF
  implementation, there is policy support, and support for Windows
  forwarding path). Updated the version to 1.2, and the date.

  \item August 2, 2006: Update the status (now XORP implements IGMPv3
  and MLDv2). Updated the version to 1.3, and the date.
