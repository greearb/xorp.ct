%
% $XORP: xorp/docs/user_manual/ospf.tex,v 1.1 2006/03/08 10:32:02 atanu Exp $
% 
...
OSPF is the Open Shortest Path First protocol it one of two principle
Interior Gateway Protocols (IGP) the other being IS-IS. RIP is also an
IGP however OSPF and IS-IS have better scaling properties.
...
an AS to split into distinct regions that OSPF calls areas. The areas
...
other protocols, these routes are introduced via
AS-external-LSAs. Routes from a RIP cloud can be introduced into OSPF
and will appear as AS-external-LSAs.
...
{\stt stub} or {\stt nssa} areas only. If the router is an Area Border
Router, then this option controls the introduction of summaries into
the area.
\begin{description}
\item{\tt disable}: This takes the value {\stt true} or {\stt
false}. The default setting is {\stt false} it can be set to {\stt
true} to disable the sending of summaries.
\end{description}
\end{description}
