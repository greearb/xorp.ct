%
% $XORP: xorp/docs/user_manual/policy.tex,v 1.21 2007/04/02 21:53:23 pavlin Exp $
%
...
user@hostname> {\bf test policy in 10.0.0.0/8}\\
...
\begin{tabbing}
xxxxxxxxxxxxxxxx\=\kill
user@hostname> {\bf test policy in2 10.0.0.0/8 ``--protocol=ospf4''}\\
...
In this case we ran the policy {\em in2} in the context of {\em ospf4}.  The
...
\begin{tabbing}
xxxxxxxxxxxxxxxx\=\kill
user@hostname> {\bf test policy med1 10.0.0.0/8 ``--protocol=bgp --med=1''}\\
Policy decision: accepted \\
user@hostname> {\bf test policy med1 10.0.0.0/8 ``--protocol=bgp --med=2''}\\
Policy decision: rejected
\end{tabbing}
\end{alltt}
\end{minipage}
