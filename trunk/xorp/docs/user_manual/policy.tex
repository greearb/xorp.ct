...
Normally policies will operate within a single routing protocol, such as setting
the MED on all BGP routes.  If a policy involves two different protocols, then
{\em route redistribution} will occur ``implicitly''.
...
as they enter the system, as they are about to leave destination and ultimately
which actions to perform on them.  The structure of a term is as follows:
...
{\em argument}.  A variable is a route attribute such as metric, prefix, nexthop
and so on.  The operator will specify {\em how} this variable is matched.  For
example $<$ will perform a less-than match whereas $>$ will perform a
greater-than operation.  The argument will be the value against which the
variable is matched.  The overall result is a {\em logical and} with the result
of each statement. An example would be as follows:
...
Actions have a similar syntax to match conditions.  All actions are performed
sequentially. The main exception is that operators will normally be assignment
and that special {\em commands} exist.  These commands are namely {\tt accept}
and {\tt reject}.  If a route is accepted, no further terms will be executed and
the route will be propagated downstream.  If a route is rejected, again no
further terms will run, and the route will {\em not} be propagated
downstream---it will be suppressed and dropped.  Depending on whether it is an
export or import filter, reject will have different semantics.  On export it
will not be advertised and on import it will never be used at all.  
...
more practical to reference a set of prefixes to match against which may be used
in different policies rather than enumerating the prefixes one by one in each
policy.  This is achieved via sets which contain un-ordered items and no
...
the case.  If a set (such as BGP communities) were matched against a set the
user specifies, depending on the operator, different semantics would apply.  For
example the sets must be equal, or one has to be the subset of the other and so
...
monitor routes flowing through XORP.  This functionality if given via policy
tracing.
...
different protocol.  An example would be to advertise some static routes with
BGP.  Or possibly some BGP routes with OSPF and so on.  The key is that the {\tt
from} block of a term will be matched in the protocol which {\em received} the
route whereas the {\tt to} block will be matched in the protocol which is {\em
advertising} the route (doing the redistribution).  Thus, route redistribution
will always be an export policy---the protocol exporting (advertising) is the
one redistributing.  All actions (such as changing the metric) will occur in the
protocol doing the redistribution.  
...
the route.\\
\hline
{\tt prefix4-length} & {\tt :} & u32range & XXX \\
{\tt prefix6-length} & {\tt :} & u32range & XXX \\
\end{tabular}
...
BGP export policy is created, the {\tt to} must be BGP by definition as it is
...
filter is placed on the RIB too.  Thus, if an export policy rejects all routes,
the RIB will never receive these routes and no routes will go in the kernel.  To
avoid this, match {\tt neighbor 0.0.0.0} in the {\tt to} block and {\tt accept}.
The next term could match all and reject.  This ``feature'' is actually useful
if you want a BGP peering but do not wish to change the routing table.
...
Variable & Operator & Argument type & Semantics \\
\hline
{\tt nexthop4} & {\tt :} & ipv4range & XXX \\
{\tt nexhtop6} & {\tt :} & ipv6range & XXX \\
\hline
...
\hline
{\tt community} & {\tt :} & txt & XXX \\
{\tt community-list} & {\tt :} & community-list & XXX \\
\hline
{\tt neighbor} & {\tt :} & ipv4range & XXX (matches who the route was learnt
from / its going to) \\
\hline
{\tt origin} & {\tt :} & u32 & Matches the origin attribute of the route (0,1,2)
XXX \\
\hline
{\tt med} & {\tt :} & u32 & Matches the MED of the route. \\
\hline
{\tt localpref} & {\tt :} & u32 & Matches the localpref of the route. \\
\end{tabular}
...
redistribution in principle.  Currently it is not enabled, although it should
work (only changed in .tp files should be required).
...
process would have a modified med.  It is also possible that advertised routes
as a consequence will have a med of 42, even though it is used as an import
policy.
...
For some reason, static routes with metric 7 are important and they are
advertised to 10.0.0.2 with MED of 7 and are also logged.  Note that 10.0.0.1
will receive these static routes with a MED of 1, even if they had a metric of
7.
