%
% $XORP: xorp/docs/pim_testsuite/pim_testsuite.tex,v 1.8 2003/02/01 00:37:03 pavlin Exp $
%
...
\begin{enumerate}
  ...
  \item Compose an Assert message at TR3 with the S address set to the
  address of S1, the RPT-bit set to zero, metric and metric preference set to
  zero, and send it on LAN3.

  \item Continue observing the (S,G) Assert state in the RUT.

  \item Compose an Assert message at TR1 with the S address set to the
  address of S1, the RPT-bit set to zero, metric and metric preference set to
  zero, and send it on LAN1.

  \item Continue observing the (S,G) Assert state in the RUT.

  \item Compose an Assert message at TR3 with the S address set to the
  address of S1, the RPT-bit set to zero, metric and metric preference set to
  zero, and send it on LAN2.
...
\subpara{Part G: Receive Preferred Assert with RPTbit clear in I Am Assert
Winner State.}
...
The results should be similar as in Part A, except that the Assert message
...
\begin{itemize}
  ...
  (\verb=Assert_Time= - \verb=Assert_Override_Interval=)
  ({\PimsmAssertTime} - {\PimsmAssertOverrideInterval}).
  the RUT should transmit Assert message on LAN1 with the RPT bit set to zero,
  the metric and metric preference set to 100. The (S,G) Assert state
...
\begin{itemize}
  ...
  = 50, the Assert state machine for the interface taht connects the RUT to
