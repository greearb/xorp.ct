%
% $XORP: xorp/docs/pim_testsuite/pim_testsuite.tex,v 1.3 2003/01/10 05:10:33 pavlin Exp $
%
...
\begin{itemize}
  ...
  \item After the MRIB is changed, the RUT should send an (*,*,RP) Join message
  to TR3 (the new upstream router toward the RP), and right after that
  it should send an (*,*,RP) Prune message to TR2 (the old upstream router
...
\para{Possible Problems:}
In Part E, after the MRIB is changed, the (*,*,RP) Prune message might
be sent before the (*,*,RP) Join message.
...
correct upstream neighbor, it should suporess its own (*,G) Join message.
...
\begin{itemize}
  ...
  \item After the MRIB is changed, the RUT should send an (*,G) Join message
  to TR3 (the new upstream router toward the RP), and right after that
  it should send an (*,G) Prune message to TR2 (the old upstream router
...
\para{Possible Problems:}
In Part E, after the MRIB is changed, the (*,G) Prune message might
be sent before the (*,G) Join message. In Part G, if the Assert generated by
