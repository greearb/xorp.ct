%
% $XORP: xorp/docs/pim_testsuite/pim_testsuite.tex,v 1.5 2003/01/12 05:03:21 pavlin Exp $
%
...
\end{verbatim}
  ...
  Join Timer in the corresponding (S,G) state to \verb=t_joinsuppress=
  (see the protocol spec for description of its value):
...
\begin{enumerate}
  ...
  \item Trigger changing of RPF'(S,G,rpt) at the RUT by composing an
  (S,G) Assert message at TR3 with RPT-bit, metric and metric 
...
\end{verbatim}
  ...
  neighbor address set to TR2. The (S,G,rpt) state at the RUT may be removed,
  but the (S,G) state must be kept until the new Assert state expires, and it
