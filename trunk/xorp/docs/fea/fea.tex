%
% $XORP: xorp/docs/fea/fea.tex,v 1.17 2006/03/09 00:18:08 pavlin Exp $
%
...
In other words, currently (August 2006) the {\tt fea} supports DragonFlyBSD,
...
\begin{itemize}
  ...
  \item March 10, 2003: Minor changes. Updated the version to 0.2, and the
	date.
  \item June 9, 2003: Updated to match the reality after the MFEA->FEA
	merging. Updated the version to 0.3, and the date.
  \item August 28, 2003: Added ``Linux /proc'' to the list of implemented
	mechanisms for reading network interface information.
	Updated the version to 0.4, and the date.
  \item November 6, 2003: Updated the FEA status.
	Updated the version to 0.5, and the date.
  \item July 8, 2004: Updated the status: now there is support for
        TCP/UDP socket I/O interface, support for libfeaclient XRL interface,
        and support for FIB observer interface.
	Updated the version to 1.0, and the date.
  \item April 13, 2005: Updated the status for the raw sockets interface and
	for Click. Updated the version to 1.1, and the date.
  \item March 8, 2006: Updated the status: now the FEA supports Windows Server
	2003, and the raw sockets interface is in use.
