...
in [XX].  This document describes an important aspect of the XRL
...
here, only low-level API was available for dispatching and handling
...
directed to do in order to be executed.  Multiple XRL targets can
...
it's name and IPC mechanisms with the Finder, so other targets are
...
information on the process hosting the target.  Each XRL Target will
implement interfaces specific to their field of operation: a routing
process would implement the ``routing process interface'' so that the
RIB process can make identity agnostic XRL calls for tasks like adding
a route.
...
\smallskip Each item in the XRL Method list consists of a method name, it's
...
and kdoc comments can be used to describe the XRL, it's arguments, and
...
are located in the \texttt{xorp/xrl/interfaces} directory.
...
argument marshalling when the XRL is dispatch and when it returns.
...
\ref{lst:eg-interface}.  The Xrl Interface Client C++ has constructor
...
received, it will be forwarded to the derived classes implementation
