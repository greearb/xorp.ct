...
source addresses and nexthops must have link-local addresses.  May use path MTU discovery for packet sizes.
...
of peers and their sets of routes.  It's timer driven and outputs 1
response packet per each time it's scheduled.  The timer expiry
interval is set to the interpacket spacing.
...
set of {\tt RouteEntry} objects can always resume from a valid point.
