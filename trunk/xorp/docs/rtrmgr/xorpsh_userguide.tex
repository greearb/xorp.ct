...
\title{XORP Shell User Guide (xorpsh) \\
...
node.
\end{itemize}
The set command is used to set or change the value of a configuration
option.  The change does not actually take effect immediately - the
commit command must be used to apply this and any other uncommitted
changes.

In the example below, the prefix length (netmask) of address
192.150.187.108 on vif xl0 is changed, but not yet committed.  The
...
New configuration can simply be typed at the command line while in
configuration mode. The user interface for this is currently rather
...
The delete command can be used to delete subtrees from the
configuration.  The deletion will be visible in the results of the
show command, but will not actually take place until the changes are
committed.
\vspace{0.1in}
...
The commit command commits all the current configuration changes.
...
original configuration, or by using the exit command to leave
configuration mode:
\vspace{0.1in}
