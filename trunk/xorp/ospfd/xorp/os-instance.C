...
/* Routines covering both the Xorp ospfd routing daemon
...
/* Constructor for the common XORP OspfSysCalls
 * class.
 */
...

{
	...
	if (FD_ISSET(fd, wrsetp) && !conn->monpkt.sendpkt()) {
	    /* XXX Should remove connection or just remove write selector ? */
	    close_monitor_connection(conn); 
	}
    }
...

{
	...
	if (!_eventloop.add_selector(fd, SEL_ALL,
				      callback(this, 
					       ...
					       conn))) {
	    XLOG_WARNING("Failed to add selector.");
	}
    }
}
...

{
	...
	XLOG_ERROR("Monitor socket failed: %s", strerror(errno));
	exit(1);
    }
	...
	XLOG_FATAL("Monitor bind failed: %s", strerror(errno));
    }
    size = sizeof(monaddr);
    if (getsockname(listenfd, (struct sockaddr *) &monaddr, &size) < 0) {
	XLOG_FATAL("getsockname: %s", strerror(errno));
    }
...
void OSInstance::accept_cb(int fd, SelectorMask m)

{
    assert(m == SEL_RD);
    assert(get_listenfd() == fd);
    accept_monitor_connection();
}

void OSInstance::tcp_data_cb(int fd, SelectorMask m, TcpConn* conn)

{
    assert(fd == conn->monfd());
    if (m & SEL_RD)
	process_monitor_request(conn);
    if ((m & SEL_WR) && !conn->monpkt.sendpkt())
	close_monitor_connection(conn);
}
