...
#ident "$XORP: xorp/xrl/interfaces/fea_fib_xif.cc,v 1.16 2008/09/23 08:13:17 abittau Exp $"
...
)
{
    ...
    if (!x) {
        ...
        x->args().add(XrlAtom(client_target_name));
        x->args().add(XrlAtom(send_updates));
        x->args().add(XrlAtom(send_resolves));
    }
