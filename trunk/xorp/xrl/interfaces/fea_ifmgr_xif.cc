...
#ident "$XORP: xorp/xrl/interfaces/fea_ifmgr_xif.cc,v 1.47 2008/10/09 17:47:02 abittau Exp $"
...
)
{
    ...
    return _sender->send(*x, callback(this, &XrlIfmgrV0p1Client::unmarshall_get_configured_baudrate, cb));
}
...
)
{
    ...
    return _sender->send(*x, callback(this, &XrlIfmgrV0p1Client::unmarshall_create_mac, cb));
}
...
)
{
    ...
    return _sender->send(*x, callback(this, &XrlIfmgrV0p1Client::unmarshall_delete_mac, cb));
}
...
)
{
    ...
    if (!x) {
        x = new Xrl(dst_xrl_target_name, "ifmgr/0.1/set_vif_vlan");
        x->args().add(XrlAtom(tid));
        x->args().add(XrlAtom(ifname));
        x->args().add(XrlAtom(vif));
