...
#ident "$XORP: xorp/xrl/interfaces/policy_xif.cc,v 1.10 2005/10/23 20:41:39 abittau Exp $"
...
{
    Xrl x(the_tgt, "policy/0.1/update_term_block");
    x.args().add("policy", policy);
    x.args().add("term", term);
...
{
    Xrl x(the_tgt, "policy/0.1/add_varmap");
    x.args().add("protocol", protocol);
...
{
    Xrl x(the_tgt, "policy/0.1/dump_state");
    x.args().add("id", id);
...
{
    Xrl x(the_tgt, "policy/0.1/set_proto_target");
    x.args().add("protocol", protocol);
