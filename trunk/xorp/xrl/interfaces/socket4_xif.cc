...
#ident "$XORP: xorp/xrl/interfaces/socket4_xif.cc,v 1.16 2007/03/28 13:49:32 schooley Exp $"
...
{
    Xrl x(dst_xrl_target_name, "socket4/0.1/send_with_flags");
    x.args().add("sockid", sockid);
    x.args().add("data", data);
...
{
    Xrl x(dst_xrl_target_name, "socket4/0.1/send_to_with_flags");
    x.args().add("sockid", sockid);
    x.args().add("remote_addr", remote_addr);
    x.args().add("remote_port", remote_port);
    x.args().add("data", data);
...
{
    Xrl x(dst_xrl_target_name, "socket4/0.1/get_socket_option");
    x.args().add("sockid", sockid);
    x.args().add("optname", optname);
