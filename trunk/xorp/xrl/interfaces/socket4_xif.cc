...
#ident "$XORP: xorp/xrl/interfaces/socket4_xif.cc,v 1.22 2008/01/04 03:18:01 pavlin Exp $"
...
{
    Xrl x(dst_xrl_target_name, "socket4/0.1/udp_open_bind_broadcast");
    x.args().add("creator", creator);
...
{
    Xrl x(dst_xrl_target_name, "socket4/0.1/udp_enable_recv");
    x.args().add("sockid", sockid);
...
{
    Xrl x(dst_xrl_target_name, "socket4/0.1/set_socket_option_txt");
    x.args().add("sockid", sockid);
    x.args().add("optname", optname);
    x.args().add("optval", optval);
