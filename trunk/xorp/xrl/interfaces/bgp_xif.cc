...
#ident "$XORP: xorp/xrl/interfaces/bgp_xif.cc,v 1.1.1.1 2002/12/11 23:56:17 hodson Exp $"
...
bool
XrlBgpV0p2Client::send_get_peer_list(
	const char*	the_tgt, 
	const CB11&	cb
)
{
    Xrl x(the_tgt, "bgp/0.2/get_peer_list");
    return _router->send(x, callback(this, &XrlBgpV0p2Client::unmarshall11, cb));
}


/* Unmarshall get_peer_list */
void
...
)
{
    ...
    cb->dispatch(e, &token, &local_ip, &local_port, &peer_ip, &peer_port, &more);
}
...
)
{
    ...
    cb->dispatch(e, &retry_interval, &hold_time, &keep_alive, &hold_time_conf, &keep_alive_conf, &min_as_orginiation_interval);
}
