...
#ident "$XORP: xorp/xrl/interfaces/bgp_xif.cc,v 1.35 2005/10/25 18:11:26 atanu Exp $"
...
bool
XrlBgpV0p2Client::send_change_peer_ip(
	const char*	the_tgt,
	const string&	local_ip,
	const uint32_t&	local_port,
	const string&	peer_ip,
	...
	const ChangePeerIpCB&	cb
)
{
    Xrl x(the_tgt, "bgp/0.2/change_peer_ip");
    x.args().add("local_ip", local_ip);
    x.args().add("local_port", local_port);
    x.args().add("peer_ip", peer_ip);
    x.args().add("peer_port", peer_port);
    x.args().add("new_peer_ip", new_peer_ip);
    return _sender->send(x, callback(this, &XrlBgpV0p2Client::unmarshall_change_peer_ip, cb));
}


/* Unmarshall change_peer_ip */
void
XrlBgpV0p2Client::unmarshall_change_peer_ip(
	const XrlError&	e,
	XrlArgs*	a,
	ChangePeerIpCB		cb
)
