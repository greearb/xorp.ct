...
#ident "$XORP: xorp/xrl/interfaces/fea_ifmgr_mirror_xif.cc,v 1.19 2007/07/16 22:29:24 pavlin Exp $"
...
bool
XrlFeaIfmgrMirrorV0p1Client::send_vif_set_vlan_tag(
	const char*	dst_xrl_target_name,
	const string&	ifname,
	const string&	vifname,
	const uint32_t&	vlan_tag,
	const VifSetVlanTagCB&	cb
)
{
    Xrl x(dst_xrl_target_name, "fea_ifmgr_mirror/0.1/vif_set_vlan_tag");
    x.args().add("ifname", ifname);
    x.args().add("vifname", vifname);
    x.args().add("vlan_tag", vlan_tag);
    return _sender->send(x, callback(this, &XrlFeaIfmgrMirrorV0p1Client::unmarshall_vif_set_vlan_tag, cb));
}


/* Unmarshall vif_set_vlan_tag */
void
XrlFeaIfmgrMirrorV0p1Client::unmarshall_vif_set_vlan_tag(
	const XrlError&	e,
	XrlArgs*	a,
	VifSetVlanTagCB		cb
)
