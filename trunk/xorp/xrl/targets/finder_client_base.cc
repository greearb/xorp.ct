...
#ident "$XORP: xorp/xrl/targets/finder_client_base.cc,v 1.3 2003/03/10 23:01:33 hodson Exp $"
...
{
    if (xrl.const_args().size() != 0) { 
	XLOG_ERROR("Wrong number of arguments (%u != 0) handling finder_client/0.1/hello", (uint32_t)xrl.const_args().size());
	return XrlCmdError::BAD_ARGS();
    }
...
XrlFinderclientTargetBase::handle_finder_client_0_1_remove_xrl_from_cache(const Xrl& xrl, XrlArgs* /* args */)
{
	...
	XLOG_ERROR("Wrong number of arguments (%u != 1) handling finder_client/0.1/remove_xrl_from_cache", (uint32_t)xrl.const_args().size());
	return XrlCmdError::BAD_ARGS();
    }
...
{
    if (xrl.const_args().size() != 1) { 
	XLOG_ERROR("Wrong number of arguments (%u != 1) handling finder_client/0.1/remove_xrls_for_target_from_cache", (uint32_t)xrl.const_args().size());
	return XrlCmdError::BAD_ARGS();
    }
