...
#ident "$XORP: xorp/xrl/targets/ripng_base.cc,v 1.13 2004/05/31 04:06:40 hodson Exp $"
...
XrlRipngTargetBase::handle_ripng_0_1_import_protocol_routes(const XrlArgs& xa_inputs, XrlArgs* /* pxa_outputs */)
{
    if (xa_inputs.size() != 3) {
	XLOG_ERROR("Wrong number of arguments (%u != %u) handling %s",
            3, (uint32_t)xa_inputs.size(), "ripng/0.1/import_protocol_routes");
	return XrlCmdError::BAD_ARGS();
    }
	...
	if (e != XrlCmdError::OKAY()) {
	    XLOG_WARNING("Handling method for %s failed: %s",
            		 "ripng/0.1/import_protocol_routes", e.str().c_str());
	    return e;
        }
    } catch (const XrlArgs::XrlAtomNotFound& e) {
	XLOG_ERROR("Argument not found");
...
XrlRipngTargetBase::handle_ripng_0_1_no_import_protocol_routes(const XrlArgs& xa_inputs, XrlArgs* /* pxa_outputs */)
{
    if (xa_inputs.size() != 1) {
	XLOG_ERROR("Wrong number of arguments (%u != %u) handling %s",
            1, (uint32_t)xa_inputs.size(), "ripng/0.1/no_import_protocol_routes");
	return XrlCmdError::BAD_ARGS();
    }
	...
	if (e != XrlCmdError::OKAY()) {
	    XLOG_WARNING("Handling method for %s failed: %s",
            		 "ripng/0.1/no_import_protocol_routes", e.str().c_str());
	    return e;
        }
    } catch (const XrlArgs::XrlAtomNotFound& e) {
	XLOG_ERROR("Argument not found");
...
XrlRipngTargetBase::add_handlers()
{
	...
	if (_cmds->add_handler("ripng/0.1/import_protocol_routes",
	    callback(this, &XrlRipngTargetBase::handle_ripng_0_1_import_protocol_routes)) == false) {
	    XLOG_ERROR("Failed to xrl handler finder://%s/%s", "ripng", "ripng/0.1/import_protocol_routes");
	}
	if (_cmds->add_handler("ripng/0.1/no_import_protocol_routes",
	    callback(this, &XrlRipngTargetBase::handle_ripng_0_1_no_import_protocol_routes)) == false) {
	    XLOG_ERROR("Failed to xrl handler finder://%s/%s", "ripng", "ripng/0.1/no_import_protocol_routes");
	}
...
XrlRipngTargetBase::remove_handlers()
{
	...
	_cmds->remove_handler("ripng/0.1/import_protocol_routes");
	_cmds->remove_handler("ripng/0.1/no_import_protocol_routes");
