...
#ident "$XORP: xorp/xrl/targets/coord_base.cc,v 1.11 2003/11/18 01:47:37 hodson Exp $"
...
{
    if (xa_inputs.size() != 6) {
	XLOG_ERROR("Wrong number of arguments (%u != 6) handling datain/0.1/receive",
            (uint32_t)xa_inputs.size());
	return XrlCmdError::BAD_ARGS();
    }
...
{
    if (xa_inputs.size() != 3) {
	XLOG_ERROR("Wrong number of arguments (%u != 3) handling datain/0.1/error",
            (uint32_t)xa_inputs.size());
	return XrlCmdError::BAD_ARGS();
    }
...
{
    if (xa_inputs.size() != 2) {
	XLOG_ERROR("Wrong number of arguments (%u != 2) handling datain/0.1/closed",
            (uint32_t)xa_inputs.size());
	return XrlCmdError::BAD_ARGS();
    }
