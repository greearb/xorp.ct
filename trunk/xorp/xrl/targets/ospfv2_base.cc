...
#ident "$XORP: xorp/xrl/targets/ospfv2_base.cc,v 1.13 2005/11/12 23:43:23 atanu Exp $"
...
const XrlCmdError
XrlOspfv2TargetBase::handle_ospfv2_0_1_set_authentication(const XrlArgs& xa_inputs, XrlArgs* /* pxa_outputs */)
{
    if (xa_inputs.size() != 5) {
	XLOG_ERROR("Wrong number of arguments (%u != %u) handling %s",
            XORP_UINT_CAST(5), XORP_UINT_CAST(xa_inputs.size()), "ospfv2/0.1/set_authentication");
	return XrlCmdError::BAD_ARGS();
    }

    /* Return value declarations */
    try {
	XrlCmdError e = ospfv2_0_1_set_authentication(
	    xa_inputs.get_string("ifname"),
	    xa_inputs.get_string("vifname"),
	    xa_inputs.get_ipv4("area"),
	    xa_inputs.get_string("type"),
	...
	if (e != XrlCmdError::OKAY()) {
	    XLOG_WARNING("Handling method for %s failed: %s",
            		 "ospfv2/0.1/set_authentication", e.str().c_str());
	    return e;
        }
    } catch (const XrlArgs::XrlAtomNotFound& e) {
	XLOG_ERROR("Argument not found");
...
XrlOspfv2TargetBase::add_handlers()
{
	...
	if (_cmds->add_handler("ospfv2/0.1/set_authentication",
	    callback(this, &XrlOspfv2TargetBase::handle_ospfv2_0_1_set_authentication)) == false) {
	    XLOG_ERROR("Failed to xrl handler finder://%s/%s", "ospfv2", "ospfv2/0.1/set_authentication");
	}
...
XrlOspfv2TargetBase::remove_handlers()
{
	...
	_cmds->remove_handler("ospfv2/0.1/set_authentication");
