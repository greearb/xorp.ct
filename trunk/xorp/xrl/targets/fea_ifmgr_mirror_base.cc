...
#ident "$XORP: xorp/xrl/targets/fea_ifmgr_mirror_base.cc,v 1.21 2007/07/12 21:46:11 pavlin Exp $"
...
const XrlCmdError
XrlFeaIfmgrMirrorTargetBase::handle_fea_ifmgr_mirror_0_1_vif_set_vlan_tag(const XrlArgs& xa_inputs, XrlArgs* /* pxa_outputs */)
{
    if (xa_inputs.size() != 3) {
	XLOG_ERROR("Wrong number of arguments (%u != %u) handling %s",
            XORP_UINT_CAST(3), XORP_UINT_CAST(xa_inputs.size()), "fea_ifmgr_mirror/0.1/vif_set_vlan_tag");
	return XrlCmdError::BAD_ARGS();
    }

    /* Return value declarations */
    try {
	XrlCmdError e = fea_ifmgr_mirror_0_1_vif_set_vlan_tag(
	    xa_inputs.get_string("ifname"),
	    xa_inputs.get_string("vifname"),
	    xa_inputs.get_uint32("vlan_tag"));
	if (e != XrlCmdError::OKAY()) {
	    XLOG_WARNING("Handling method for %s failed: %s",
            		 "fea_ifmgr_mirror/0.1/vif_set_vlan_tag", e.str().c_str());
	    return e;
        }
    } catch (const XrlArgs::BadArgs& e) {
	XLOG_ERROR("Error decoding the arguments: %s", e.str().c_str());
...
XrlFeaIfmgrMirrorTargetBase::add_handlers()
{
	...
	if (_cmds->add_handler("fea_ifmgr_mirror/0.1/vif_set_vlan_tag",
	    callback(this, &XrlFeaIfmgrMirrorTargetBase::handle_fea_ifmgr_mirror_0_1_vif_set_vlan_tag)) == false) {
	    XLOG_ERROR("Failed to xrl handler finder://%s/%s", "fea_ifmgr_mirror", "fea_ifmgr_mirror/0.1/vif_set_vlan_tag");
	}
...
XrlFeaIfmgrMirrorTargetBase::remove_handlers()
{
	...
	_cmds->remove_handler("fea_ifmgr_mirror/0.1/vif_set_vlan_tag");
