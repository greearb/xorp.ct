...
#ident "$XORP: xorp/xrl/targets/profiler_base.cc,v 1.14 2009/01/05 18:31:15 jtc Exp $"


#include "profiler_base.hh"


const struct XrlProfilerTargetBase::handler_table XrlProfilerTargetBase::handlers[] = {
    { "common/0.1/get_target_name",
      &XrlProfilerTargetBase::handle_common_0_1_get_target_name },
    { "common/0.1/get_version",
      &XrlProfilerTargetBase::handle_common_0_1_get_version },
    { "common/0.1/get_status",
      &XrlProfilerTargetBase::handle_common_0_1_get_status },
    { "common/0.1/shutdown",
      &XrlProfilerTargetBase::handle_common_0_1_shutdown },
    { "profile_client/0.1/log",
      &XrlProfilerTargetBase::handle_profile_client_0_1_log },
    { "profile_client/0.1/finished",
      &XrlProfilerTargetBase::handle_profile_client_0_1_finished },
};

const size_t XrlProfilerTargetBase::num_handlers = (sizeof(XrlProfilerTargetBase::handlers) / sizeof(XrlProfilerTargetBase::handlers[0]));
...
XrlProfilerTargetBase::handle_profile_client_0_1_log(const XrlArgs& xa_inputs, XrlArgs* /* pxa_outputs */)
{
    if (xa_inputs.size() != 4) {
	XLOG_ERROR("Wrong number of arguments (%u != %u) handling %s",
            XORP_UINT_CAST(4), XORP_UINT_CAST(xa_inputs.size()), "profile_client/0.1/log");
	return XrlCmdError::BAD_ARGS();
    }
	...
	XrlCmdError e = profile_client_0_1_log(
	    ...
	    xa_inputs.get(1, "sec").uint32(),
	    xa_inputs.get(2, "usec").uint32(),
	    xa_inputs.get(3, "comment").text());
	if (e != XrlCmdError::OKAY()) {
	    XLOG_WARNING("Handling method for %s failed: %s",
...
XrlProfilerTargetBase::handle_profile_client_0_1_finished(const XrlArgs& xa_inputs, XrlArgs* /* pxa_outputs */)
{
    if (xa_inputs.size() != 1) {
	XLOG_ERROR("Wrong number of arguments (%u != %u) handling %s",
            XORP_UINT_CAST(1), XORP_UINT_CAST(xa_inputs.size()), "profile_client/0.1/finished");
	return XrlCmdError::BAD_ARGS();
    }
...
XrlProfilerTargetBase::add_handlers()
{
    for (size_t i = 0; i < num_handlers; ++i) {
        if (!_cmds->add_handler(handlers[i].name,
            ...
            XLOG_ERROR("Failed to register xrl handler finder://%s/%s", "profiler", handlers[i].name);
        }
    }
    _cmds->finalize();
}
