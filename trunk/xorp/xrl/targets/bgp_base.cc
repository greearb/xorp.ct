...
#ident "$XORP: xorp/xrl/targets/bgp_base.cc,v 1.40 2005/10/25 18:11:27 atanu Exp $"
...
const XrlCmdError
XrlBgpTargetBase::handle_bgp_0_2_change_peer_ip(const XrlArgs& xa_inputs, XrlArgs* /* pxa_outputs */)
{
    if (xa_inputs.size() != 5) {
	XLOG_ERROR("Wrong number of arguments (%u != %u) handling %s",
            XORP_UINT_CAST(5), XORP_UINT_CAST(xa_inputs.size()), "bgp/0.2/change_peer_ip");
	return XrlCmdError::BAD_ARGS();
    }

    /* Return value declarations */
    try {
	XrlCmdError e = bgp_0_2_change_peer_ip(
	    xa_inputs.get_string("local_ip"),
	    xa_inputs.get_uint32("local_port"),
	    xa_inputs.get_string("peer_ip"),
	    xa_inputs.get_uint32("peer_port"),
	    xa_inputs.get_string("new_peer_ip"));
	if (e != XrlCmdError::OKAY()) {
	    XLOG_WARNING("Handling method for %s failed: %s",
            		 "bgp/0.2/change_peer_ip", e.str().c_str());
	    return e;
        }
    } catch (const XrlArgs::XrlAtomNotFound& e) {
	XLOG_ERROR("Argument not found");
...
XrlBgpTargetBase::add_handlers()
{
	...
	if (_cmds->add_handler("bgp/0.2/change_peer_ip",
	    callback(this, &XrlBgpTargetBase::handle_bgp_0_2_change_peer_ip)) == false) {
	    XLOG_ERROR("Failed to xrl handler finder://%s/%s", "bgp", "bgp/0.2/change_peer_ip");
	}
...
XrlBgpTargetBase::remove_handlers()
{
	...
	_cmds->remove_handler("bgp/0.2/change_peer_ip");
