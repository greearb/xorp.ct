...
const XrlCmdError
XrlBgpTargetBase::handle_bgp_0_2_set_bgpid(const XrlArgs& xa_inputs, XrlArgs* /* pxa_outputs */)
{
    if (xa_inputs.size() != 1) {
	XLOG_ERROR("Wrong number of arguments (%u != 1) handling bgp/0.2/set_bgpid",
            (uint32_t)xa_inputs.size());
	return XrlCmdError::BAD_ARGS();
    }
...
const XrlCmdError
XrlBgpTargetBase::handle_bgp_0_2_get_bgpid(const XrlArgs& xa_inputs, XrlArgs* pxa_outputs)
{
    if (xa_inputs.size() != 0) {
	XLOG_ERROR("Wrong number of arguments (%u != 0) handling bgp/0.2/get_bgpid",
            (uint32_t)xa_inputs.size());
	return XrlCmdError::BAD_ARGS();
    }
