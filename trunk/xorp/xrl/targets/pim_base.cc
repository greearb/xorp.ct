...
#ident "$XORP: xorp/xrl/targets/pim_base.cc,v 1.32 2004/03/02 00:29:46 pavlin Exp $"
...
{
    if (xa_inputs.size() != 8) {
	XLOG_ERROR("Wrong number of arguments (%u != 8) handling redist_transaction4/0.1/add_route",
            (uint32_t)xa_inputs.size());
	return XrlCmdError::BAD_ARGS();
    }
...
{
    if (xa_inputs.size() != 8) {
	XLOG_ERROR("Wrong number of arguments (%u != 8) handling redist_transaction6/0.1/add_route",
            (uint32_t)xa_inputs.size());
	return XrlCmdError::BAD_ARGS();
    }
