...
#ident "$XORP: xorp/xrl/targets/policy_base.cc,v 1.19 2008/08/06 08:23:26 abittau Exp $"
...
XrlPolicyTargetBase::add_handlers()
{
	...
	if (_cmds->add_handler("cli_processor/0.1/process_command",
	    callback(this, &XrlPolicyTargetBase::handle_cli_processor_0_1_process_command)) == false) {
	    XLOG_ERROR("Failed to xrl handler finder://%s/%s", "policy", "cli_processor/0.1/process_command");
	}
	_cmds->finalize();
}
