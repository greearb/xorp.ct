...
#ident "$XORP: xorp/xrl/targets/rip_base.cc,v 1.39 2008/10/29 21:59:41 andrewma Exp $"
...
XrlRipTargetBase::add_handlers()
{
	...
	if (_cmds->add_handler("rip/0.1/trace",
	    callback(this, &XrlRipTargetBase::handle_rip_0_1_trace)) == false) {
	    XLOG_ERROR("Failed to xrl handler finder://%s/%s", "rip", "rip/0.1/trace");
	}
	...
	if (_cmds->add_handler("socket4_user/0.1/inbound_connect_event",
	    callback(this, &XrlRipTargetBase::handle_socket4_user_0_1_inbound_connect_event)) == false) {
	    XLOG_ERROR("Failed to xrl handler finder://%s/%s", "rip", "socket4_user/0.1/inbound_connect_event");
	}
	if (_cmds->add_handler("socket4_user/0.1/outgoing_connect_event",
	    callback(this, &XrlRipTargetBase::handle_socket4_user_0_1_outgoing_connect_event)) == false) {
	    XLOG_ERROR("Failed to xrl handler finder://%s/%s", "rip", "socket4_user/0.1/outgoing_connect_event");
	}
	...
	if (_cmds->add_handler("socket4_user/0.1/disconnect_event",
	    callback(this, &XrlRipTargetBase::handle_socket4_user_0_1_disconnect_event)) == false) {
	    XLOG_ERROR("Failed to xrl handler finder://%s/%s", "rip", "socket4_user/0.1/disconnect_event");
	}
	_cmds->finalize();
}
...
XrlRipTargetBase::remove_handlers()
{
	...
	_cmds->remove_handler("rip/0.1/trace");
