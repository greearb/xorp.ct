...
#ident "$XORP: xorp/xrl/targets/rip6_base.cc,v 1.2 2003/11/18 01:47:42 hodson Exp $"


#include "rip6_base.hh"


XrlRip6TargetBase::XrlRip6TargetBase(XrlCmdMap* cmds)
    : _cmds(cmds)
{
    if (_cmds)
	add_handlers();
...
XrlRip6TargetBase::~XrlRip6TargetBase()
{
    if (_cmds)
	remove_handlers();
}
...
bool
XrlRip6TargetBase::set_command_map(XrlCmdMap* cmds)
{
    if (_cmds == 0 && cmds) {
        _cmds = cmds;
        add_handlers();
