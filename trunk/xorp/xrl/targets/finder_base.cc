...
#ident "$XORP: xorp/xrl/targets/finder_base.cc,v 1.8 2003/03/10 23:01:33 hodson Exp $"
...
const XrlCmdError
XrlFinderTargetBase::handle_finder_0_1_set_finder_client_enabled(const Xrl& xrl, XrlArgs* /* args */)
{
    if (xrl.const_args().size() != 2) { 
	XLOG_ERROR("Wrong number of arguments (%u != 2) handling finder/0.1/set_finder_client_enabled", (uint32_t)xrl.const_args().size());
	return XrlCmdError::BAD_ARGS();
    }

    /* Return value declarations */
    try {
	XrlCmdError e = finder_0_1_set_finder_client_enabled(
	    xrl.const_args().get_string("target_name"), 
	    xrl.const_args().get_bool("enabled"));
	if (e != XrlCmdError::OKAY()) {
	    XLOG_WARNING("Handling method for finder/0.1/set_finder_client_enabled failed: %s \"%s\"",
            		 e.str().c_str(), e.note().c_str());
	    return e;
        }
    } catch (const XrlArgs::XrlAtomNotFound& e) {
...
const XrlCmdError
XrlFinderTargetBase::handle_finder_0_1_finder_client_enabled(const Xrl& xrl, XrlArgs* args)
{
    if (xrl.const_args().size() != 1) { 
	XLOG_ERROR("Wrong number of arguments (%u != 1) handling finder/0.1/finder_client_enabled", (uint32_t)xrl.const_args().size());
	return XrlCmdError::BAD_ARGS();
    }
    ...
    bool enabled; 
    try {
	XrlCmdError e = finder_0_1_finder_client_enabled(
	    xrl.const_args().get_string("target_name"), 
	    enabled);
	if (e != XrlCmdError::OKAY()) {
	    XLOG_WARNING("Handling method for finder/0.1/finder_client_enabled failed: %s \"%s\"",
            		 e.str().c_str(), e.note().c_str());
	    return e;
        }
    } catch (const XrlArgs::XrlAtomNotFound& e) {
