...
#ident "$XORP: xorp/xrl/targets/rip4_base.cc,v 1.4 2004/01/09 00:17:05 hodson Exp $"
...
const XrlCmdError
XrlRip4TargetBase::handle_rip4_0_1_add_static_route(const XrlArgs& xa_inputs, XrlArgs* /* pxa_outputs */)
{
    if (xa_inputs.size() != 3) {
	XLOG_ERROR("Wrong number of arguments (%u != 3) handling rip4/0.1/add_static_route",
            (uint32_t)xa_inputs.size());
	return XrlCmdError::BAD_ARGS();
    }
    ...
    /* Return value declarations */
    try {
	XrlCmdError e = rip4_0_1_add_static_route(
	    xa_inputs.get_ipv4net("network"),
	    xa_inputs.get_ipv4("nexthop"),
	    xa_inputs.get_uint32("cost"));
	if (e != XrlCmdError::OKAY()) {
	    XLOG_WARNING("Handling method for rip4/0.1/add_static_route failed: %s",
            		 e.str().c_str());
	    return e;
        }
    } catch (const XrlArgs::XrlAtomNotFound& e) {
...
const XrlCmdError
XrlRip4TargetBase::handle_rip4_0_1_delete_static_route(const XrlArgs& xa_inputs, XrlArgs* /* pxa_outputs */)
{
    if (xa_inputs.size() != 1) {
	XLOG_ERROR("Wrong number of arguments (%u != 1) handling rip4/0.1/delete_static_route",
            (uint32_t)xa_inputs.size());
	return XrlCmdError::BAD_ARGS();
    }
    ...
    /* Return value declarations */
    try {
	XrlCmdError e = rip4_0_1_delete_static_route(
	    xa_inputs.get_ipv4net("network"));
	if (e != XrlCmdError::OKAY()) {
	    XLOG_WARNING("Handling method for rip4/0.1/delete_static_route failed: %s",
            		 e.str().c_str());
	    return e;
        }
    } catch (const XrlArgs::XrlAtomNotFound& e) {
