...
#ident "$XORP: xorp/xrl/targets/fea_base.cc,v 1.5 2003/03/22 03:26:43 pavlin Exp $"
...
XrlFeaTargetBase::handle_ifmgr_0_1_get_vif_flags4(const Xrl& xrl, XrlArgs* args)
{
	...
	XLOG_ERROR("Wrong number of arguments (%u != 3) handling ifmgr/0.1/get_vif_flags4", (uint32_t)xrl.const_args().size());
	return XrlCmdError::BAD_ARGS();
    }
...
{
    if (xrl.const_args().size() != 3) { 
	XLOG_ERROR("Wrong number of arguments (%u != 3) handling ifmgr/0.1/get_vif_flags6", (uint32_t)xrl.const_args().size());
	return XrlCmdError::BAD_ARGS();
    }
