#!/usr/bin/perl -w
#
# $XORP: CVSROOT/log_accum.pl,v 1.9 2004/07/31 21:33:19 pavlin Exp $
# $FreeBSD: CVSROOT-src/log_accum.pl,v 1.128 2004/06/05 11:02:42 des Exp $
#
	...
	foreach my $line (@lines) {
		if ($line =~ /.*\/$/) {
			...
			if ($tag eq 'HEAD') {
				...
				push @text, sprintf("%-21s (Branch: %s)",
				    "$header files:", $tag);
			}
		} else {
			push @files, $line;
...
sub build_header {
	...
	my $hostdomain = $hostname;
	if ($hostdomain eq "") {
	    $hostdomain = $domainname;
	} elsif ($domainname ne ".") {
	    $hostdomain = $hostdomain . $domainname;
	}
	my $committer = sprintf("%s@%s", $cfg::COMMITTER, $hostdomain);

	my $datestr = strftime("%F %T %Z", localtime());
	chomp $datestr;

	my @path = split('/', $ARGV[0]);
	my $modulename = $path[0];	# I.e. the module name == top-level dir

	my $header = sprintf("CVSROOT:\t%s\nModule name:\t%s\nChanges by:\t%s\t%s",
			     $CVSROOT,
			     $modulename,
			     $committer,
			     $datestr);
...
#	print(MAIL "\n");
	...
	my $subject = "Subject: $cfg::MAIL_SUBJECT_PREAMBLE";
