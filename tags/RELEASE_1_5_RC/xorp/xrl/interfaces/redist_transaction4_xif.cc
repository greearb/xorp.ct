...
#ident "$XORP: xorp/xrl/interfaces/redist_transaction4_xif.cc,v 1.15 2007/07/12 21:46:09 pavlin Exp $"
...
{
    Xrl x(dst_xrl_target_name, "redist_transaction4/0.1/start_transaction");
...
{
    Xrl x(dst_xrl_target_name, "redist_transaction4/0.1/commit_transaction");
    x.args().add("tid", tid);
...
{
    Xrl x(dst_xrl_target_name, "redist_transaction4/0.1/abort_transaction");
    x.args().add("tid", tid);
...
{
    Xrl x(dst_xrl_target_name, "redist_transaction4/0.1/add_route");
    x.args().add("tid", tid);
    x.args().add("dst", dst);
...
{
    Xrl x(dst_xrl_target_name, "redist_transaction4/0.1/delete_route");
    x.args().add("tid", tid);
    x.args().add("dst", dst);
    x.args().add("nexthop", nexthop);
    x.args().add("ifname", ifname);
...
{
    Xrl x(dst_xrl_target_name, "redist_transaction4/0.1/delete_all_routes");
    x.args().add("tid", tid);
    x.args().add("cookie", cookie);
