...
#ident "$XORP: xorp/xrl/interfaces/finder_xif.cc,v 1.27 2008/09/23 08:13:18 abittau Exp $"
...
)
{
    ...
    if (!x) {
        ...
        x->args().add(XrlAtom(singleton));
        x->args().add(XrlAtom(in_cookie));
    }
...
)
{
    ...
    if (!x) {
        ...
        x->args().add(XrlAtom(protocol_args));
    }
