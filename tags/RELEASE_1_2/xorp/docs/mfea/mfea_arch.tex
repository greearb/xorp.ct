%
% $XORP: xorp/docs/mfea/mfea_arch.tex,v 1.20 2005/11/29 01:48:37 pavlin Exp $
%
...
Currently (March 2006), the MFEA supports abstraction for
the following systems:

\begin{itemize}
  \item \{Free,Net,Open\}BSD OS
  \item Linux OS
\end{itemize}

In the future, the MFEA will support simulation environment, and
abstraction for Click~\cite{CLICK-PROJECT} forwarding
path~\footnote{Currently (March 2006) Click does
...
(March 2006) there are few occasions when XrlMfeaNode uses direct access to
MfeaVif.}.
...
(March 2006), MfeaConfig is not implemented; rather, all state is
...
\begin{itemize}
  ...
  \item January 27, 2005: Remove MRIB-related text, because the MFEA
  does not deal with the MRIB information anymore.

  \item April 13, 2005: Updated to match XORP version 1.1 release code.
