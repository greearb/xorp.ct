%
% $XORP: xorp/docs/pim/pim_arch.tex,v 1.26 2002/12/11 10:19:33 pavlin Exp $
%
...
Currently (December 2002), the PIM-SM implementation is based
on the specification in the following documents:
\begin{itemize}
  \item \verb=draft-ietf-pim-sm-v2-new-05.{ps,txt}= (The core PIM-SM
  specification).
  \item \verb=draft-ietf-pim-sm-bsr-02.{ps,txt}= (The Bootstrap mechanism).
\end{itemize}
...
implementation (December 2002) XrlPimNode for simplicity uses
...
(December 2002) the PimScopeZoneTable is used only for PIM
...
For simplicity of implementation, currently (December 2002) PimMrt
...
the multicast forwarding engine. Currently (December 2002), those
...
possible to create such table manually. However, currently (December
2002) there are 47 input events and 70 output operations,
...
elapsed time is above a threshold (default to 100ms as of December
2002), we save the appropriate state to continue the processing later
...
PimBsr keeps two lists of BsrZone entries: one list for the active BSR
zones, and another list for the locally configured Cand-BSR zones and/or
Cand-RP information. On startup, the active BSR zones list is empty. If
the node is configured as a Candidate-BSR and/or a Candidate-RP, this
information will be added to the second list; otherwise that list is
also empty.
...
timeout, the RpTable is updated accordingly.
...
The RpTable may contain one special PimRp entry with address of
...
of them may map to this new RP. Those who do map are moved to the
appropriate list on the new RP.
...
module~\footnote{Currently (December 2002), the information is
...
Note that as we mentioned earlier, currently (December 2002), the MRIB
...
(December 2002), PimConfig is not implemented; rather, all state is
...
\begin{itemize}
  ...
  \item PimScopeZone table: add information about scoped zones.
