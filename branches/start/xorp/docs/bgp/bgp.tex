...
{\tt delete\_route}, or {\tt replace\_route} calls.  Each batch of calls is for
...
The current (Nov 2002) version of XORP stores all routes in the RibIn,
...
reach the BGP NextHop (which is often a number of IP hops away).  Also
of interest is whether the BGP NextHop is actually reachable according
to the IGP protocols.  Because of the multi-process architecture of
XORP, BGP does not know the IGP distance or whether the nexthop is
reachable.  To find out this information, BGP must query the RIB, and
this is done by the NextHopResolver class instance.  If DecisionTable
had to perform this lookup, it would become very complex because it
would have to handle suspending the decision process while waiting for
results from the RIB.  A multi-threaded implementation would solve
this problem, but would cause other issues.  
...
RouteTable class.  A BGP implementation has a single NextHopResolver
...
(RFC????) could be handled efficiently.  In our current implementation
...
event loop.  Execution will the be restarted after pending network
...
\item An iterator pointing to the next RibIn trie entry to be dumped
on the current peer (we call this the Route Iterator)
\item A flag that indicates whether the Route Iterator is currently
valid.
\item The subnet of the route pointed to by the Route Iterator.
\item The version of the RibIn of the current peer when we started to
dump it.  RibIn version number are incremented each time a change is
made to the RibIn.
...
The DumpTable calls {\tt dump\_next\_route} on it's parent table, passing the
...
If the RibIn version has not changed since the last call of {\tt
dump\_next\_route}, then the Route Iterator is still valid, and the
route it references can be directly dumped.  If not, then a search is
performed for the first route in the trie with a subnet equal to or
greater than the subnet stored in the DumpIterator, and
this is the route that will be dumped.  This allows routes in the
RibIn to be deleted during the route dump without causing a problem.
...
In general, a route change needs to be propagated downstream if:
\begin{itemize}
